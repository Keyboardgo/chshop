...
if(isset($_GET['kw']) && !empty($_GET['kw'])) {
	...
	<div class="wrapper">';
		...
		$numrows=$DB->getColumn("SELECT COUNT(*) FROM pre_orders A WHERE{$sql}");
		$ondate=$DB->getColumn("SELECT COUNT(*) FROM pre_orders A WHERE A.status=1 AND{$sql}");
		$ondate2=$DB->getColumn("SELECT COUNT(*) FROM pre_orders A WHERE A.status=2 AND{$sql}");
		$con='
...
// 现在每个订单都有唯一的tradeno，所以不需要GROUP BY
$rs=$DB->query("SELECT 
    A.id,
    A.tradeno,
    A.tid,
    ...
    A.money as total_money,
    1 as order_count,
    A.input as all_inputs,
    A.status as all_status,
    A.djzt as all_djzt,
    B.name
FROM pre_orders A 
LEFT JOIN pre_tools B ON A.tid=B.tid 
WHERE{$sql} 
ORDER BY A.id DESC 
LIMIT $offset,$pagesize");
	while($res = $rs->fetch())
	{
		...
		// 处理输入信息显示
		if($res['all_djzt']==3 && $res['userid']!=$userrow['zid']){
			$input_display = '****'.substr($res['all_inputs'],4);
		}else{
			$input_display = $res['all_inputs'];
		}
		
		// 处理状态显示
		$status_display = display_zt($res['all_status']);
...
<script src="//lib.baomitu.com/bootstrap-datepicker/1.9.0/js/locales/bootstrap-datepicker.zh-CN.min.js"></script>
<script>
