...
function getPoint() {
	...
	var goods_sid = $('#tid option:selected').attr('goods_sid');
	$('#alert_frame').html('');
	if(desc!='' && alert!='null'){
		$('#alert_frame').show();
	...
	if(inputname=='hide'){// 关键值初始化
	$('#inputsname').append('<input type="hidden" name="inputvalue" id="inputvalue" value="'+getCookie('mysid')+'"/>');
	}else{
	...
	if($('#tid option:selected').attr('isfaka')==1){
		...
		if(getCookie('email'))$('#inputvalue').val(getCookie('email'));
	...
	$.ajax({
		...
		success : function(data) {
			...
			if(data.code == 0){
				...
				if(data.desc){
					item += '<tr><td colspan="6" style="text-align:center" class="orderTitle"><b>商品简介</b>'+(data.ts == 1 ?'<a href="./user/workorder2.php?my=add&orderid='+id+'&skey='+skey+'&ts=1" target="_blank" onclick="return checklogin('+data.islogin+')" class="btn btn-xs btn-danger">网盘链接失效投诉</a>':'')+'</td><tr><td colspan="6" class="orderContent">'+data.desc+'</td></tr>';
				}
				item += '</table>';
	...
	$.ajax({
		...
		success : function(data) {
			...
			if(data.code == 0){
				...
				$("#tid").append('<option value="0">请选择商品</option>');
				$.each(data.data, function (i, res) {
					$("#tid").append('<option value="'+res.tid+'" cid="'+res.cid+'" price="'+res.price+'" desc="'+escape(res.desc)+'" goods_sid="'+res.goods_sid+'" alert="'+escape(res.alert)+'" inputname="'+res.input+'" inputsname="'+res.inputs+'" multi="'+res.multi+'" isfaka="'+res.isfaka+'" isinvitegift="'+res.isinvitegift+'" invitegift_money="'+res.invitegift_money+'" invite_gift="'+res.invite_gift+'" count="'+res.value+'" close="'+res.close+'" prices="'+res.prices+'" max="'+res.max+'" min="'+res.min+'" stock="'+res.stock+'">'+res.name+'</option>');
					num++;
				});
				...
				if(num==0 && cid!=0){
					...
					layer.msg('该分类下没有找到激活的商品', {icon: 2, time: 1500});
				}else{
					layer.msg('成功加载 '+num+' 个商品', {icon: 1, time: 1000});
				}
			}else{
				layer.alert('商品加载失败: '+data.msg);
			}
		},
		error:function(xhr, status, error){
			layer.close(ii);
			layer.msg('网络请求失败: '+status+' - '+error, {icon: 2, time: 2000});
			console.error('AJAX错误:', status, error);
			console.log('响应内容:', xhr.responseText);
			// 恢复之前的cid值
			if(lastValidCid){
				$("#cid").val(lastValidCid);
			}
			return false;
		}
	});
...
// 替换jQuery.cookie调用为原生document.cookie操作
...
// 替换原始代码，避免使用modal方法
if(getCookie('sec_defend_time'))removeCookie('sec_defend_time', { path: '/' });
if( !getCookie('op') && isModal==true){
	// 使用原生JavaScript替代modal方法
	var myModal = document.getElementById('myModal');
	if(myModal) {
		// 这里可以添加简单的模态框显示逻辑
		try {
			// 尝试使用jQuery modal（如果可用）
	...
	setCookie('op', false, { expires: cookietime });
...
// 设置cookie过期时间为30天
	...
	setCookie("counter", visits, { expires: expiryDate });
