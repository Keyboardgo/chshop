...
function sendOrderEmail($row, $shoprow) {
    ...
    $mailconf = $DB->getRow("SELECT * FROM shua_config WHERE k='mail_config' limit 1");
    ...
    if($result['error'] == 0){
        $DB->query("update shua_orders set email_sent=1 where id='" . $row['id'] . "'");
        return true;
    }
    return false;
}
...
case 'setStatus':
	...
	if($status==5){
		if($DB->exec("DELETE FROM shua_orders WHERE id='$id'"))
			exit('{"code":200}');
		else
			exit('{"code":400,"msg":"删除订单失败！'.$DB->error().'"}');
	}else{
		if($DB->exec("update shua_orders set status='$status',result=NULL where id='{$id}'"))
			exit('{"code":200}');
		else
...
case 'order':
	adminpermission('order', 2);
	$id=intval($_GET['id']);
	$rows=$DB->getRow("select * from shua_orders where id='$id' limit 1");
	if(!$rows)
		exit('{"code":-1,"msg":"当前订单不存在！"}');
	$tool=$DB->getRow("select * from shua_tools where tid='{$rows['tid']}' limit 1");
	if(strpos($rows['tradeno'],'kid')!==false){
		$kid=explode(':',$rows['tradeno']);
		$kid=$kid[1];
		$trade=$DB->getRow("select * from shua_kms where kid='$kid' limit 1");
		$trade['type']='卡密';
		$addstr='<li class="list-group-item"><b>使用卡密：</b>'.$trade['km'].'</li>';
	}elseif(strpos($rows['tradeno'],'invite')!==false){
		$trade['type']='推广赠送';
	}elseif(!empty($rows['tradeno'])){
		$trade=$DB->getRow("select * from shua_pay where trade_no='{$rows['tradeno']}' limit 1");
...
case 'editOrder':
	...
	$sds=$DB->exec("update `shua_orders` set `input`='$inputvalue',`input2`='$inputvalue2',`input3`='$inputvalue3',`input4`='$inputvalue4',`input5`='$inputvalue5' where `id`='$id'");
	if($sds!==false)
...
case 'editOrderNum':
	...
	$sds=$DB->exec("update `shua_orders` set `value`='$num' where `id`='$id'");
	if($sds!==false)
		exit('{"code":0,"msg":"修改订单成功！"}');
	else
		exit('{"code":-1,"msg":"修改订单失败！'.$DB->error().'"}');
...
case 'operation':
	...
	foreach($checkbox as $id){
		...
		if($status==4)$DB->exec("DELETE FROM shua_orders WHERE id='$id'");
		elseif($status==5){
			$result = do_goods($id);
		}elseif($status==6){
			...
			if($row && $row['zid']>1 && $row['status']==3 && is_numeric($row['userid'])){
				...
				rollbackPoint($id);
				$DB->exec("update shua_orders set status='4',result=NULL where id='{\$id}'");
			}
		}
		else $DB->exec("update shua_orders set status='$status' where id='$id' limit 1");
		$i++;
	}
...
case 'setresult':
	...
	if($DB->exec("update shua_orders set result='$result' where id='{\$id}'")!==false)
		exit('{"code":0,"msg":"修改订单成功！"}');
	else
		exit('{"code":-1,"msg":"修改订单失败！'.$DB->error().'"}');
break;
...
case 'refund': //退款操作
	...
	if($tool['is_curl'] == 4 && $tool['goods_sid'] != 0){
	    $trade=$DB->getRow("select * from shua_pay where trade_no='{\$row['tradeno']}' limit 1");
...
case 'showStatus': //订单进度查询
	...
	if(is_array($list)){
		...
		if(($list['order_state']=='已完成'||$list['order_state']=='订单已完成'||$list['订单状态']=='已完成'||$list['订单状态']=='已发货'||$list['订单状态']=='交易成功'||$list['订单状态']=='已支付') && $row['status']==2){
			...
			// 订单完成后发送邮件
			$shoprow = $DB->getRow("SELECT * FROM shua_tools WHERE tid='" . $row['tid'] . "' LIMIT 1");
...
case 'fillPayOrder':
	...
	$srow = $DB->getRow("SELECT * FROM shua_pay WHERE trade_no=:trade_no", [':trade_no'=>$trade_no]);
	if (!$srow) exit(json_encode(array('code'=>-1,'msg'=>'记录不存在')));
	if($srow['status']==0){
		if($DB->exec("UPDATE `shua_pay` SET `status`='1',`endtime`=NOW() WHERE `trade_no`=:trade_no", [':trade_no'=>$trade_no])){
...
case 'delPayOrder':
	$trade_no = trim($_POST['trade_no']);
	if($DB->exec("DELETE FROM shua_pay WHERE trade_no=:trade_no", [':trade_no'=>$trade_no])!==false)
