...
if (isset($_GET["zid"])) {
	...
	if ($power && $power["power"] == 0) {
		exit("<script language='javascript'>top.location.href='./userlist.php?zid=" . $_GET["zid"] . "';</script>");
	}
	$sql = " zid=" . $_GET["zid"] . " and power>0";
} elseif (isset($_GET["power"])) {
	$sql = " power=" . $_GET["power"];
	$link = "&power=" . $_GET["power"];
} elseif (isset($_GET["mod"]) && $_GET["mod"] == "mj") {
	$sql = " iprice!=''";
	$link = "&mod=mj";
	...
	$sql = " (user='" . $_GET["kw"] . "' or zid='" . $_GET["kw"] . "' or domain='" . $_GET["kw"] . "' or domain2='" . $_GET["kw"] . "' or qq='" . $_GET["kw"] . "') and power>0";
	$link = "&kw=" . $_GET["kw"];
} else {
	$sql = " power>0";
}
