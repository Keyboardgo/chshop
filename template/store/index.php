...
// 获取所有一级分类
$re = $DB->query("SELECT * FROM `pre_class` WHERE `active` = 1 AND `pid` = 0 ORDER BY `sort` ASC ");
$qcid = "";
$cat_name = "";
...
foreach ($ar_data as $primary) {
    ...
    while ($sub_res = $sub_rs->fetch()) {
        if($is_fenzhan && in_array($sub_res['cid'], $classhide))continue;
        $sub_categories[] = $sub_res;
        // 如果当前选中的是二级分类，设置相应的信息
        if($sub_res['cid'] == $cid){
            $cat_name = $sub_res['name'];
            $qcid = $cid;
        }
    }
...
.content-slide .shop_active {    
    color: #ff5555;    
    border-bottom: 2px solid #ff5555;    
}
.sub-category.shop_active {    
    color: #ff5555;    
    background-color: #fff0f0;    
    border-radius: 5px;    
}
.secondary-category.shop_active .icon-title {
    color: #ff5555;
}
        ...
        <div class="fui-page  fui-page-current " style="height:auto; overflow: inherit">
            <div class="fui-content navbar" id="container" style="background-color: #fafafc;overflow: inherit">
                <div class="default-items">
                    ...
                    <div class="device">
                        <!-- 一级分类轮播 -->
                        <div class="swiper-container" id="primary-swiper">
                            <div class="swiper-wrapper"
                                ...
                                foreach ($ar_data as $v) {
                                    ...
                                    echo '<a data-cid="'.$v['cid'].'" data-name="'.$v['name'].'" class="get_cat primary-category">
                                               <div class="mbg">
                    ...
                    <!-- 二级分类容器 -->
                    <div id="secondary-category-container" class="category-container secondary-category-box" style="display: none; background-color: #fff; padding: 10px;">
                        <div id="secondary-categories" class="sub-categories" style="display: flex;">
                            <!-- 二级分类内容将通过JavaScript动态生成 -->
                        </div>
                    </div>
                    
                    <script>
                        // 二级分类数据
                        var subCategories = <?php echo json_encode($sub_classes); ?>;
                        ...
                        $('.primary-category').click(function() {
                            ...
                            // 检查是否有对应的二级分类
                            if (subCategories[cid] && subCategories[cid].length > 0) {
                                // 清空并生成二级分类
                                var secondaryHtml = '';
                                for (var i = 0; i < subCategories[cid].length; i++) {
                                    var subCat = subCategories[cid][i];
                                    var activeClass = (subCat.cid == <?php echo $cid; ?>) ? ' shop_active' : '';
                                    secondaryHtml += '<a data-cid="' + subCat.cid + '" data-name="' + subCat.name + '" data-primary-cid="' + cid + '" class="get_cat sub-category' + activeClass + '" style="margin: 5px 10px; padding: 5px 15px; background-color: #f5f5f5; border-radius: 15px; font-size: 14px;">';
                                    secondaryHtml += subCat.name;
                                    secondaryHtml += '</a>';
                                }
                                
                                $('#secondary-categories').html(secondaryHtml);
                                
                                // 显示二级分类容器
                                $('#secondary-category-container').show();
                                
                                // 为二级分类绑定点击事件
                                $('.sub-category').unbind('click').click(function(e) {
                                    // 阻止事件冒泡，避免触发一级分类的点击事件
                                    e.stopPropagation();
                                    
                                    var subCid = $(this).data('cid');
                                    ...
                                    // 设置选中的分类ID和名称
                                    $("input[name=_cid]").val(subCid);
                                    $("input[name=_cidname]").val(subName);
                                    
                                    // 优先调用get_goods函数加载商品，如果不存在则执行页面跳转
                                    if (typeof get_goods !== 'undefined') {
                                        get_goods();
                                    } else {
                                        window.location.href = './?cid=' + subCid;
                                    }
                                });
                                ...
                                // 隐藏二级分类容器
                                $('#secondary-category-container').hide();
                                ...
                                // 不执行页面跳转，只设置分类信息
                                // 如果需要加载当前一级分类的商品，可以在这里调用get_goods()函数
                                if (typeof get_goods !== 'undefined') {
                                    get_goods();
                                }
                            }
                        ...
                        $(document).ready(function() {
                            ...
                            var primaryCategories = $('.primary-category');
                            ...
                            if (primaryCategories.length > 0) {
                                ...
                                if (currentCid > 0) {
                                    // 查找当前分类的父分类ID
                                    var primaryCid = 0;
                                    for (var pid in subCategories) {
                                        for (var i = 0; i < subCategories[pid].length; i++) {
                                            if (subCategories[pid][i].cid == currentCid) {
                                                primaryCid = pid;
                                                break;
                                            }
                                        }
                                        if (primaryCid > 0) break;
                                    }
                                    
                                    // 如果找到了父分类，触发父分类的点击事件以显示二级分类
                                    if (primaryCid > 0) {
                                        var parentCategory = $('.primary-category[data-cid="' + primaryCid + '"]');
                                        if (parentCategory.length > 0) {
                                            parentCategory.addClass('shop_active');
                                            parentCategory.trigger('click');
                                        }
                                    } else {
                                        // 否则，直接触发当前分类的点击事件
                                        var currentCategory = $('.primary-category[data-cid="' + currentCid + '"]');
                                        if (currentCategory.length > 0) {
                                            currentCategory.addClass('shop_active');
                            ...
                            function initializeFirstCategory() {
                                var firstCategory = primaryCategories.first();
                                var firstCid = firstCategory.data('cid');
                                
                                // 为第一个一级分类添加激活状态
                                firstCategory.addClass('shop_active');
                                
                                // 设置默认分类ID和名称
                                $("input[name=_cid]").val(firstCid);
                                $("input[name=_cidname]").val(firstCategory.data('name'));
                                
                                // 检查是否有二级分类
                                if (subCategories[firstCid] && subCategories[firstCid].length > 0) {
                                    // 模拟点击第一个一级分类，显示二级分类
                                    setTimeout(function() {
                                        firstCategory.trigger('click');
                                    }, 100);
                                } else {
                                    // 如果没有二级分类，直接加载一级分类的商品
                                    if (typeof get_goods !== 'undefined') {
                                        get_goods();
                                    }
                                }
