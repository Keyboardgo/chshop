...
// 根据channel判断是充值还是商品购买
if($srow['channel'] == 'recharge'){
    // 充值渠道，使用recharge类型获取配置
    $pay_config = get_pay_api('recharge');
}else{
    // 商品购买，使用对应支付方式获取配置
...
if($verify_result && ($conf['alipay_api']==2 || $conf['qqpay_api']==2 || $conf['wxpay_api']==2 || $conf['qqpay_api']==8 || $conf['wxpay_api']==8 || $conf['wxpay_api']==9 || $conf['recharge_api']==2 || $conf['recharge_api']==8 || $conf['recharge_api']==10) && !empty($pay_config['pid']) && !empty($pay_config['key'])) {//验证成功
    ...
    if ($_GET['trade_status'] == 'TRADE_SUCCESS' && $srow['status']==0 && round($srow['money'],2)==round($money,2)) {
		...
		if($DB->exec("UPDATE `pre_pay` SET `status` ='1' WHERE `trade_no`='{$out_trade_no}'")){
			...
			// 更新支付记录状态
			$DB->exec("UPDATE `pre_payment_records` SET `status`=1,`endtime`=:endtime,`api_trade_no`=:api_trade_no WHERE `trade_no`=:trade_no", [
				':endtime' => $date,
