...
switch ($act) {
	case 'payrmb':
		...
		if ($srow['status'] == 0) {
			...
			if ($srow['tid'] == -3) {
				// 先扣费
				if ($DB->exec("UPDATE `pre_site` SET `rmb`=`rmb`-'{$srow['money']}' WHERE `zid`='{$userrow['zid']}'")) {
					// 更新订单状态为已支付
					$DB->exec("UPDATE `pre_pay` SET `type`='rmb',`status`='1',`endtime`=NOW() WHERE `trade_no`='{$orderid}'");
					$srow['type'] = 'rmb';
					$srow['status'] = 1;
					...
					if (processOrder($srow)) {
						...
						// 如果处理失败，回滚扣费
						$DB->exec("UPDATE `pre_site` SET `rmb`=`rmb`+'{$srow['money']}' WHERE `zid`='{$userrow['zid']}'");
						$DB->exec("UPDATE `pre_pay` SET `type`='',`status`='0',`endtime`=NULL WHERE `trade_no`='{$orderid}'");
...
//					}
	...
	case 'order': //订单进度查询
		...
		if ($tool['is_curl'] == 4 || $row['djzt'] == 3) {
            ...
            if($tool['goods_type'] == '1' && $shequ['type'] == 'daishua'){
                $kmdata = $row['result'];
            }
		...
		$result = array('code' => 0, 'msg' => 'succ', 'name' => $tool['name'], 'money' => $row['money'], 'date' => $row['addtime'], 'inputs' => $inputsdata, 'list' => $list, 'kminfo' => $kmdata, 'alert' => $tool['alert'], 'desc' => $tool['desc'], 'status' => $row['status'], 'result' => $row['result'], 'complain' => intval($conf['show_complain']), 'islogin' => $islogin2, 'selfrefund' => $conf['selfrefund'],'ts'=>$tool['ts']);
		exit(json_encode($result));
		break;
	case 'apply_refund':
	...
	case 'invite_create':
		...
		if (isset($conf['fanghong_url']) && !empty($conf['fanghong_url'])) $url = fanghongdwz($url);
		$content = str_replace('[url]', $url['url'], $conf['invite_content']);
		if (!$content) $content = $url;
		$result = array('code' => 0, 'msg' => 'succ', 'url' => $url['url'], 'content' => $content);
		exit(json_encode($result));
		break;
	case 'invite_verify':
		...
		if ($invite_row && $invite_row['status'] == 0) {
			...
			if ($shop && $shop['active'] == 1 && $shop['type'] == 1) {
				...
				if ($DB->getColumn("SELECT count(*) FROM `pre_invitelog` WHERE `iid`=:iid AND `ip`=:ip", [':iid' => $invite_row['id'], ':ip' => $clientip]) == 0) {
					$DB->exec("INSERT INTO `pre_invitelog`(`iid`,`type`,`date`,`ip`,`status`) VALUES (:iid, 1, NOW(), :ip, 1)", [':iid' => $invite_row['id'], ':ip' => $clientip]);
	...
	case 'cart_buy':
		...
		$toolnames = array();
		foreach ($ids as $id) {
			$cart_item_temp = $DB->getRow("SELECT * FROM `pre_cart` WHERE `id`='$id' LIMIT 1");
			$tool_temp = $DB->getRow("SELECT * FROM pre_tools WHERE tid='{$cart_item_temp['tid']}' LIMIT 1");
			$toolnames[] = $tool_temp['name'];
		}
		$toolname = implode('、', array_unique($toolnames));
		if (count(array_unique($toolnames)) > 1) {
			$toolname .= '等多件';
		}
		$input = implode('|', $ids);
		$trade_no = date("YmdHis") . rand(111, 999);
	...
	case 'cart_cancel':
		...
		// 如果订单已支付，需要退款
		if ($srow['status'] == 1 && $srow['type'] == 'rmb' && $srow['money'] > 0) {
			// 退款给用户
			$DB->exec("UPDATE `pre_site` SET `rmb`=`rmb`+'{$srow['money']}' WHERE `zid`='{$srow['zid']}'");
			addPointRecord($srow['zid'], $srow['money'], '退款', '取消订单 ' . $srow['name'] . ' (' . $orderid . ')');
		}
		...
		if ($srow['status'] == 0) {
			...
			$input = explode('|', $srow['input']);
			$ids = implode(',', $input);
			$DB->exec("UPDATE pre_cart SET status=0 WHERE id IN ($ids) AND status=2");
		}
		
		// 删除支付记录
		$DB->exec("DELETE FROM pre_pay WHERE trade_no='{$orderid}'");
		
		exit('{"code":0,"msg":"订单已取消"}');
		break;
	case 'cart_empty':
		$userid = $islogin2 ? $userrow['zid'] : $cookiesid;
		if ($DB->exec("DELETE FROM pre_cart WHERE userid='$userid' AND status=0") !== false) {
